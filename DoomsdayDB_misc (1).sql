...
-- SPROC to update information when a person's health reaches 0.
...
--Adds deceased person's wealth to their corresponding faction's coffer
    ...
    FROM
        DoomsdayDatabaseTest.dbo.Person p
    JOIN DoomsdayDatabaseTest.dbo.Faction f ON p.FactionID = f.FactionID
    JOIN DoomsdayDatabaseTest.dbo.CurrencyPerson cp ON p.PersonID = cp.PersonID
    JOIN DoomsdayDatabaseTest.dbo.Currency c ON c.CurrencyID = cp.CurrencyID
...
FROM DoomsdayDatabaseTest.dbo.Faction f
...
FROM
    DoomsdayDatabaseTest.dbo.CurrencyPerson cp
    JOIN DoomsdayDatabaseTest.dbo.Person p ON p.PersonID = cp.PersonID
WHERE
    p.PersonDeceased = 1;
